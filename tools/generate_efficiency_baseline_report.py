...
def build_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## 单字排序策略对比",
        "",
        f"这组对比固定使用同一份单字真实频率作为需求权重，只替换单字桶内的排序策略：`tier_only` 表示只按当前按新频率量级动态定标的 9 档分层骨架排序（当前档间步长 `{format_float(tier_step)}`），`tier_plus_frequency` 表示在同样 9 档骨架上继续叠加单字真频率，`tier_plus_frequency_plus_modern_common` 表示在此基础上再叠加“现代常用单字优先”的轻量约束，`current_runtime` 表示继续叠加读音纠偏与词语读音先验后的当前排序。",
        "",
        "| 策略 | 有效编码桶数 | 真实频率加权首选命中率 | 真实频率加权首屏可见率 |",
        "| --- | ---: | ---: | ---: |",
        f"| 仅 9 档分层 | {char_ordering_comparison['tier_only']['bucket_count']} | {format_pct(float(char_ordering_comparison['tier_only']['weighted_top1_share']))} | {format_pct(float(char_ordering_comparison['tier_only']['weighted_first_page_share']))} |",
        f"| 9 档 + 真单字频率 | {char_ordering_comparison['tier_plus_frequency']['bucket_count']} | {format_pct(float(char_ordering_comparison['tier_plus_frequency']['weighted_top1_share']))} | {format_pct(float(char_ordering_comparison['tier_plus_frequency']['weighted_first_page_share']))} |",
        f"| 9 档 + 真单字频率 + 现代常用轻量约束 | {char_ordering_comparison['tier_plus_frequency_plus_modern_common']['bucket_count']} | {format_pct(float(char_ordering_comparison['tier_plus_frequency_plus_modern_common']['weighted_top1_share']))} | {format_pct(float(char_ordering_comparison['tier_plus_frequency_plus_modern_common']['weighted_first_page_share']))} |",
        ...
        f"| 真频率相对仅 9 档改善 | - | {format_pct(float(char_ordering_comparison['delta_tier_plus_frequency_vs_tier_only']['weighted_top1_share']))} | {format_pct(float(char_ordering_comparison['delta_tier_plus_frequency_vs_tier_only']['weighted_first_page_share']))} |",
        ...
        f"| 现代常用轻量约束相对仅 9 档总改善 | - | {format_pct(float(char_ordering_comparison['delta_modern_common_vs_tier_only']['weighted_top1_share']))} | {format_pct(float(char_ordering_comparison['delta_modern_common_vs_tier_only']['weighted_first_page_share']))} |",
        f"| 当前 runtime 相对仅 9 档总改善 | - | {format_pct(float(char_ordering_comparison['delta_current_runtime_vs_tier_only']['weighted_top1_share']))} | {format_pct(float(char_ordering_comparison['delta_current_runtime_vs_tier_only']['weighted_first_page_share']))} |",
        "",
        "## 同语料码长对照基准",
        "",
        "这部分不是第三方输入法实测，而是同一批运行时语料上的结构码长基线。它只回答“同一批条目如果按不同编码长度模型输入，理论键数大约是多少”。",
        "",
        "比较口径：`音元等长模式`每个音节固定 4 键；`音元变长模式`保留实首音或虚首音，并合并组成干音的相邻相同音元；`音元省键模式`从变长结果继续派生，并在同音质的 `高-中-低` 或 `低-中-高` 三音序列中省略中间的中调乐音；`标准全拼`按无调拼音字母数累计；`带数字调号标准全拼`在无调字母数基础上为每个带调音节再计 1 个数字键；`抽象双拼`每个音节固定 2 键。",
        "",
        f"- 同语料总体上，音元省键模式平均码长为 `{format_float(float(code_length_baseline['overall']['avg_yime_jianpin']))}` 键，较音元等长模式平均减少 `{format_float(float(code_length_baseline['overall']['jianpin_saved_vs_full']))}` 键。",
    ...
    lines.extend(
        [
            "## 省键模式样本分析",
            "",
            "下面只看单个音节的结构压缩效果。这里的‘压缩最明显’与‘几乎压不动’只依据两条压缩规则：合并组成干音的相邻相同音元，以及省略符合条件的干音中调；实首音和虚首音都固定保留。分析不涉及候选排序或学习成本。",
