...
3. 候选框在光标附近出现
4. 选择候选：
  首选候选可用 `Space`、`Enter` 或鼠标左键。
  第 2~5 候选可依次用 `` ` ``、`-`、`=`、`\`，也可用鼠标左键。
  任意候选都可先用方向键移动高亮，再按 `Space` / `Enter`；也可直接鼠标左键点击。
  如果每页候选数设为 6~9，第 6~9 候选建议用鼠标左键，或先用方向键定位后再按 `Space` / `Enter`。
...
### Q16：`N01-N27` 和 `M01-M33` 到底是什么？

它们是系统的 **Yinyuan ID**：为每一种音元编定的稳定唯一标识，
不是可删的临时中间产物。完整的 `N01`、`M01` 才是 ID；它们不等于
Unicode 码点、键位或四元编码位置。
...
- `syllable/codec/yinjie_code.json`、各类 `*_codepoint.json`、`yinyuan.klc` 更接近生成产物层
