...
2. “片音”对应 `phonic slice`，表示从时域亦即音值上切分出来的音片（语音片段），而不完全是传统意义上的音段或音素 `phonetic segment`。
...
#### 真源主链：`syllable.codec.yinjie_decoder.YinjieDecoder`

带调拼音 → 四码 → ``Yinjie`` 的 canonical 编解码真源。IME 运行时主链已转向“变长音元码优先查词”，但四码仍是结构与映射的真源。常用方法：
...
| `split_encoded_string(encoded)` | 切分四字符音元编码串                 |
...
四元模型递归结构树；四码 canonical 编解码真源。IME 运行时查词与连续输入可使用变长音元码主链，但它们仍以这里的四个编码位结构映射为上游真源。术语见 [TERMINOLOGY_INDEX.md](TERMINOLOGY_INDEX.md)。
...
| `classify_yinyuan_chars()` / `classify_codes()` | 噪音 / 乐音分类 |
| `classify_phonemes()` | 旧兼容名；新代码优先用 `classify_yinyuan_chars()` |
...
| `yime.utils.rime_export` | Rime 分发导出工具；从运行时物化表导出 schema/dict 测试数据 |
