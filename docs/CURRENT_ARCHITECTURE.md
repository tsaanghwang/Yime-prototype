...
> **2026-08 脱离产品维护：** 本文只描述当前仓库中可复现的历史原型、来源和审计链。
> Yime 的现行产品真源、Windows 构建和发布均在 `C:\dev\Yime`；本仓库不再提供产品交接。
> 详细边界见[脱离产品维护边界](DETACHED_MAINTENANCE_BOUNDARY.md)。
>
> **本仓库内部的 2026-07 数据切换：** `.generated/lexicon_source_bundle/source_lexicon.sqlite3`
> 是这里进行来源核对、历史重建和恢复研究时的字词、读音、证据、频次及音元编码输入真源。
...
Unihan 单字读音 / phrase-pinyin-data 词语读音 / 经审查补丁
  ...
  -> source_lexicon.sqlite3（本仓库内部重建真源）
  ...
  -> SQLite 研究候选及本仓库内的历史消费产物
```

三模式严格按同一条派生链生成：等长模式保留每音节四个音元；第一位是实首音或虚首音，后三位是组成干音的呼音、主音和末音。变长模式固定保留首音，只合并组成干音的相邻相同音元：三者不同时保持不变，只有前两者相同时合并前两者，只有后两者相同时合并后两者，三者相同时合并为一个音元。省键模式以变长结果为输入，只对仍由三个同音质音元构成、且调级为高—中—低或低—中—高的干音省略中间的中调音元。虚首音承担连续输入的音节边界，不再作为省键项删除。与 Windows Yime `codemode` 的一致性只作为历史兼容证据，不构成本仓库向产品仓库交接或同步的接口。
...
| 字词、读音、频次、九级汉字分级与来源证据 | `.generated/lexicon_source_bundle/source_lexicon.sqlite3` | 本仓库内部重建真源；统一门禁后的编码输入边界及 `character_tiers` 分级表 |
...
- 本仓库保留语义、生成和审计实现用于历史核对与恢复研究，但不再是 Yime 产品主仓库，也不再向
  Windows 输入法、Rime 或 KLC 自动同步。产品侧的编码、布局、构建和发布工作只在
  `C:\dev\Yime` 独立审查和实施。
...
202,290、全量 pypinyin、A/B 和 B-lite 等容量档继续作为历史对照。下列 Windows Yime 默认发布层
数据是脱离产品维护前的历史快照，不是本仓库当前支持的发布配置：

- 1,152,157个不同字串、1,167,057条不重复的已编码运行映射；
- 46,095个已编码单字全部进入运行链，其中14,000字为核心、32,095字为低频外围；
- 当时的安装包只携带核心词典、schema、manifest 和运行 profile；
