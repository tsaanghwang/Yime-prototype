...
- `temp.json`
  ...
  - 原因：其内容已由 `syllable/yinyuan/zaoyin_yinyuan_enhanced.json` 等现行文件承载，保留根目录副本只会制造“哪个才是当前文件”的歧义。
...
- `phoneme_dict.json`
  - 分类：已删除的根目录再生导出物。
  - 原因：仓库内没有任何运行时读取方；它只由 `syllable/codec/yinjie_decoder.py` 导出，适合下沉到 `yime/reports/phoneme_dict.json` 这类报告路径，而不应继续占据根目录主资产位置。
...
- `ganyin_encoding.json`
  - 分类：已删除的根目录孤立干音编码快照。
  - 原因：这份根目录文件没有任何消费入口，也不在现行验证链中；而后续保留在 `syllable/yinyuan/` 的同名切片副本也已确认只是历史快照，不再属于当前活动链路。
...
- `yime/windows_candidate_box.py`
  - 分类：旧候选框测试壳兼容入口。
  - 原因：当前主线候选框实现已位于 `yime/input_method/ui/`，该模块不再是正式启动入口；真实实现现已归档到 `yime/legacy/pending_removal/windows_candidate_box.py`，根路径仅保留兼容 shim 以避免打断历史调用方式。
...
- `start_yinjie_encoder.bat`
  - 分类：已删除的根目录手动启动包装脚本。
  - 原因：仓库内没有任何代码、文档、测试或其它脚本再引用它；功能只是包一层 `syllable/codec/yinjie_encoder.py`，与直接运行 Python 入口重复。
