...
未来可信来源复核；该状态不等同于噪声、永久拒绝或最终不予编码。候选模型仍兼容读取旧版
`nonencoding_strings` 表，但按新的暂缓语义迁移，不再恢复为 `noise + reject`。
...
候选。现行分类包括人名、商号/企业字号、品名、货币计量、其他专名、固定词项、噪声和无法判定。
...
这三个按钮用于固定词、例外、噪声及证据不足材料的逐条处理；判断为能产组合时应优先使用规则族
