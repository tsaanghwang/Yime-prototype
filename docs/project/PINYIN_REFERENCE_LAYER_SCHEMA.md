...
- `yime/legacy/pending_removal/Initialize_pinyin_mapping.py`
  - 隔离后的 legacy 初始化脚本，现已只建并写入 `多式拼音映射关系`
- `yime/legacy/pending_removal/split_numeric_pinyin.py`
  - 隔离后的数字调拼音导入逻辑，现已只把 `多式拼音映射关系` 作为资料层来源
