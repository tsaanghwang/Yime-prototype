...
音元输入法(YIME)是以音元为码元的汉语音码输入系统，基于独创的音元分析理论构建。这套系统通过 52 个音元（22 个充当首音的噪音类音元和30 个构成干音的乐音类音元）实现高效汉字输入，相比传统拼音输入法具有以下优势：

- **更短的编码长度**：音节平均码长比任何现行全拼输入法的平均码长都短
...
- **首音系统**：首音只由噪音充当，包括实首音和虚首音，共有22个。首音对应音节声母。实首音对应非零声母。虚首音对应零声母。
- **干音系统**：干音只由乐音构成。干音指由声调与韵母构成的音段。干音对应带调韵母。
...
一种是重排键位的美式键盘。这种键盘布局，在全拼状态下，按照一音一码原则，在52个音元中，把47个音元对应的音符映射到下档键位上，把5个音元对应的音符映射到上档键位上。在这种键盘上，音元分布草图如图：
...
在音元输入法中，根据一音一码原则确定音元与键位的对应关系。在通用现代汉语中，首音由噪音充当。换句话说，首音就是噪音。首音与声母一一对应。首音 ng[ŋ]对应开口呼零声母或隔音符号，在美式键盘上，用小写字母键 v 键或其它字母键输入，以便做到完全不用符号键（除键值为 65\~90 和 97\~122 的键位外的键位）输入音元。
