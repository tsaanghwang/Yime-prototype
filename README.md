...
一种是重排键位的美式键盘。这种键盘布局，在全拼状态下，按照一音一码原则，在52个音元中，把47个音元映射到下档键位上，把5个音元映射到上档键位上。在这种键盘上，音元分布草图如图：
...
一种是一音一键的新式键盘。这种键盘的布局是，通过输入中英文切换键改变输入状态，在中文输入状态下，把键值为 65\~90 和 97\~122 的键位（对应美式键盘的大小写字母键）全部做成下档键并建立音元与键位的映射关系。这就是说，这种键盘共有 52 个下档键位：键值为 65\~90 和 97\~122 的键位；其它键值的键位，包括数字 0\~9、运算符号、标点符号、等等，全部做成上档键位。这种键盘主要用来根据音元输入法输入汉字。
...
在音元输入法中，若用 ASCII 码编码，根据键位排序，在美式键盘上，键位对应的音元——上档和下档键位对应的音元列表见表：
