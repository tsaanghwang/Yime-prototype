...
音元输入法(YIME)是一套以音元为码元的汉语音码输入系统，基于独创的音元分析理论构建。该系统通过 52 个音元（22 个充当首音噪音+30 个构成干音的乐音）实现高效汉字输入，相比传统拼音输入法具有以下优势：

- **更短的编码长度**：音节平均码长比任何全拼输入法的平均码长都短
- **更高的输入效率**：单字输入和词语输入平均按键次数更少
...
- **首音系统**：22 个噪音类音元。噪音充当首音。首音对应音节声母，包括实首音(对应非零声母)和虚首音(对应零声母)。
- **干音系统**：30 个乐音类音元。乐音构成干音。干音对应带调韵母。乐音按音高分为高调、中调和低调三类。干音指由声调与韵母构成的音段。
...
一种是重排键位的美式键盘。这种键盘布局，在全拼状态下，按照一音一码原则，对52个音元，把47个音元对应的音符安排在初始（下档）状态下，把5个音元对应的音符安排在上档（Shift）状态下。在这种键盘上，音元分布草图如图：
