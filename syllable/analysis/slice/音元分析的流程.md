...
graph LR
   ...
   subgraph 首音和干音的分析["首音和干音的分析"]
    subgraph 首音分析["两类首音分析"]
        XuShouyin --> |析取|虚首音的音调[首调] --> |取默认值|零声母[零声母]
        XuShouyin --> |析取|虚首音的音质[首质] --> |实质上是|零声母[零声母]--> |更名|虚首音[虚首音]
        ShiShouyin --> |析取|实首音的音调[首调] --> |取默认值|非零声母[非零声母 ]
        ShiShouyin --> |析取|实首音的音质[首质] --> |实质上是|非零声母[非零声母] --> |更名|实首音[实首音]
      end

      subgraph DecomposingSingleQualityGanyin["单质干音分析"]
      ...
      subgraph DecomposingBackLongGanyin["后长干音分析"]
      ...
      subgraph DecomposingFrontLongGanyin["前长干音分析"]
      ...
      subgraph DecomposingTriQualityGanyin["三质干音分析"]
   ...
   虚首音[虚首音] --> |分析|Zaoyin[噪音]
   实首音[实首音] --> |分析|Zaoyin[噪音]
