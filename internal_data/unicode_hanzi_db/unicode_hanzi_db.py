...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "legacy_source_file",
        nargs="?",
        help="Legacy positional unicode_hanzi.txt path; equivalent to --source-file",
    )
    parser.add_argument(
        "--source-file",
    ...
    if not args.source_file:
        args.source_file = args.legacy_source_file
    if not args.source_file:
        parser.error("the following arguments are required: --source-file")
    return args
...
def build_invalid_pinyin_rows(pinyin_candidates_json: str) -> list[tuple[str, str, str]]:
    ...
    parsed: Any
    try:
        parsed = json.loads(pinyin_candidates_json) if pinyin_candidates_json else []
    except json.JSONDecodeError:
        parsed = []

    raw_candidates = cast(list[Any], parsed) if isinstance(parsed, list) else []
...
def classify_invalid_pinyin(marked: str, numeric: str, reason: str) -> str:
    ...
    if not any(char in TONE_MARK_CHARS for char in marked):
        return "plain_untoned_or_nonstandard"
...
def build_db(db_path: Path, source_file: Path) -> None:
    ...
    cur.execute("DROP VIEW IF EXISTS view_hanzi_inspection")
    cur.execute("DROP VIEW IF EXISTS view_hanzi_invalid_pinyin")
    cur.execute("DROP VIEW IF EXISTS view_hanzi_invalid_pinyin_summary")
    cur.execute("DROP VIEW IF EXISTS view_hanzi_invalid_pinyin_multisyllable")
    cur.execute("DROP VIEW IF EXISTS view_hanzi_invalid_pinyin_plain_untoned")
    cur.execute("DROP VIEW IF EXISTS view_hanzi_invalid_pinyin_toned_outside")
    cur.execute("DROP TABLE IF EXISTS hanzi_invalid_pinyin_cache")
...
def create_views(db_path: Path) -> None:
    ...
    cur.execute("""
        CREATE VIEW view_hanzi_inspection AS
        SELECT
            codepoint,
            hanzi,
            block,
            ...
            pinyin_candidates,
            CASE
                WHEN pinyin_candidates IS NULL OR pinyin_candidates = '' THEN 0
                ELSE json_array_length(pinyin_candidates)
            END AS candidate_count,
            CASE
                WHEN TRIM(COALESCE(pinyin, '')) = '' THEN 0
                ELSE 1
            END AS has_pinyin,
            CASE
                WHEN pinyin_candidates IS NOT NULL
                     AND pinyin_candidates <> ''
                     AND json_array_length(pinyin_candidates) > 1 THEN 1
                ELSE 0
            END AS is_polyphonic,
            json_extract(pinyin_candidates, '$[0]') AS candidate_1,
            json_extract(pinyin_candidates, '$[1]') AS candidate_2,
            json_extract(pinyin_candidates, '$[2]') AS candidate_3,
            CASE
                WHEN frequency >= 1000000 THEN 'very_high'
                WHEN frequency >= 100000 THEN 'high'
                WHEN frequency >= 10000 THEN 'medium'
    ...
    cur.execute("""
        CREATE TABLE hanzi_invalid_pinyin_cache (
    ...
    invalid_rows_to_insert: list[tuple[str, str, str, int, str, int, str, str, str, str]] = []
    for codepoint, hanzi, pinyin, pinyin_candidates, frequency, block in cur.execute(
        "SELECT codepoint, hanzi, pinyin, pinyin_candidates, frequency, block FROM hanzi"
    ):
        parsed_candidates: Any
        try:
            parsed_candidates = json.loads(pinyin_candidates) if pinyin_candidates else []
        except json.JSONDecodeError:
            parsed_candidates = []
    ...
    if invalid_rows_to_insert:
        cur.executemany(
            """
            INSERT INTO hanzi_invalid_pinyin_cache (
                codepoint,
                hanzi,
                block,
                frequency,
    ...
    cur.execute("""
        CREATE VIEW view_hanzi_invalid_pinyin AS
        SELECT
            codepoint,
            hanzi,
            block,
            ...
            invalid_reason,
            invalid_group
        FROM hanzi_invalid_pinyin_cache
    """)

    cur.execute("""
        CREATE VIEW view_hanzi_invalid_pinyin_summary AS
        SELECT
            invalid_group,
            invalid_reason,
            COUNT(*) AS row_count,
    ...
    cur.execute("""
        CREATE VIEW view_hanzi_invalid_pinyin_multisyllable AS
        SELECT *
        FROM view_hanzi_invalid_pinyin
        WHERE invalid_group = 'multi_syllable_or_compound'
        ORDER BY frequency DESC, codepoint ASC, candidate_rank ASC
    """)

    cur.execute("""
        CREATE VIEW view_hanzi_invalid_pinyin_plain_untoned AS
        SELECT *
        FROM view_hanzi_invalid_pinyin
        WHERE invalid_group = 'plain_untoned_or_nonstandard'
        ORDER BY frequency DESC, codepoint ASC, candidate_rank ASC
    """)

    cur.execute("""
        CREATE VIEW view_hanzi_invalid_pinyin_toned_outside AS
        SELECT *
        FROM view_hanzi_invalid_pinyin
        WHERE invalid_group = 'toned_but_outside_codebook'
        ORDER BY frequency DESC, codepoint ASC, candidate_rank ASC
    """)
    ...
    cur.execute(
        ...
        "FROM view_hanzi_inspection ORDER BY frequency DESC, codepoint ASC LIMIT 5"
    )
    console_print(f"view_hanzi_inspection 前5个: {cur.fetchall()}")

    cur.execute(
        "SELECT hanzi, primary_pinyin, invalid_marked_pinyin, invalid_reason "
        "FROM view_hanzi_invalid_pinyin ORDER BY frequency DESC, codepoint ASC LIMIT 5"
    )
    console_print(f"view_hanzi_invalid_pinyin 前5个: {cur.fetchall()}")

    cur.execute("SELECT invalid_group, invalid_reason, row_count FROM view_hanzi_invalid_pinyin_summary")
    console_print(f"view_hanzi_invalid_pinyin_summary: {cur.fetchall()}")
