...
- 授权用途限定为：将 YIME 集成到被许可方运营的 SaaS、云服务、托管平台或在线系统中。
- 服务名称：________________
- 服务域名或主要部署环境：________________
...
- 是否允许生产环境部署：允许 / 不允许
- 是否允许测试、预发布和灾备环境部署：允许 / 不允许
- 是否允许多租户使用：允许 / 不允许
- 是否允许面向第三方客户提供访问：允许 / 不允许
- 是否允许闭源部署：允许 / 不允许
- 是否允许关联主体共同运营：允许 / 不允许
...
- Testing, staging, and disaster-recovery deployment: allowed / not allowed
- Multi-tenant operation: allowed / not allowed
- Access by third-party customers: allowed / not allowed
- Closed-source deployment: allowed / not allowed
- Joint operation by affiliates: allowed / not allowed

## 5. 授权限制

- 未经书面同意，不得把本授权扩展到其他 SaaS 产品或其他品牌服务。
- 未经书面同意，不得允许客户单独提取、转售或再分发 YIME 本身。
- 若客户数量、调用规模或部署区域超出约定范围，应另行扩展授权。
...
- 终止后的停用、删除、迁移或客户通知义务：________________
...
- 是否提供缺陷修复：是 / 否
- 是否提供升级支持：是 / 否
- 是否提供 SLA 或响应时间约定：是 / 否

## 8. Maintenance, Support, and Upgrades

- Defect fixes: yes / no
- Upgrade support: yes / no
- SLA or response-time commitments: yes / no

## 9. 知识产权、保密与标识

- YIME 及其原始代码、文档和标识的知识产权归许可方所有。
- 是否将源代码、接口文档、内部部署信息纳入保密范围：是 / 否
- 是否允许在市场宣传中提及 YIME 或许可方名称：允许 / 不允许
...
- 被许可方负责其 SaaS 中客户数据、运营数据与合规责任。
- 除非另有书面约定，许可方不承担 SaaS 业务连续性、客户赔偿或监管合规义务。
