        ...
        for tone in tones:
            ...
            if syllable in pinyin_dict:
                ...
                if consonant == "r":
                    ...
                    # 无论er_syllable是否存在都进行替换
                    pinyin_dict[er_syllable] = pinyin_dict.pop(syllable)
                    print(f'已替换音节"{syllable}"为"{er_syllable}"')

    if found_syllables:
        print(f"字典中存在由调号与成音节的辅音构成的音节: {', '.join(found_syllables)}")

    # 检查只包含成音节辅音的音节
    if any(consonant in pinyin_dict for consonant in syllabic_consonants):
...
def check_eh_syllables(pinyin_dict):
    ...
    for eh_syllable in eh_syllables:
        ...
        # 直接替换为 ê+tone，因为我们已经确保 ê+tone 存在
        pinyin_dict[e_syllable] = pinyin_dict.pop(
            eh_syllable).replace(eh_syllable, e_syllable)
        print(f'已替换音节"{eh_syllable}"为"{e_syllable}"')
