...
```mermaid
graph TB
  Start((Start)) --> |Input|A[Syllable]
  subgraph ExtractToneAndQuality["Extracting the tone and quality of the syllable"]
    A --> |Extract the tone of the syllable|B[Syllabic Tone<br>Tone of the Syllable]
    A --> |Extract the quality of the syllable|C[Syllabic Quality<br>Initial Consonant and Final]
  end

  subgraph DivideToneAndQuality["Dividing the tonal and qualitative layers"]
    subgraph DivideTone["Dividing the tonal layer"]
    B --> |Extract the tone of the Initial Sound<br>Extract the tonal segment connected to the Initial Consonant|D1[Initial Tonal Segment<br>Tonal segment connected to the Initial Consonant]
    B --> |Extract the tone of the Subsequent Sound<br>Extract the tonal segment connected to the Final|D2[Subsequent Tonal Segment<br>Tonal segment connected to the Final]
    end
    subgraph DivideQuality["Dividing the qualitative layer"]
    C --> |Extract the quality of the Initial Sound<br>Extract the Initial Consonant|E1[Initial Consonant<br>Quality of the Initial Sound]
    C --> |Extract the quality of the Subsequent Sound<br>Extract the Final|E2[Final<br>Quality of the Subsequent Sound]
    end
  end

  subgraph DivideInitialAndSubsequent["Dividing the Initial Sound and the Subsequent Sound"]
    subgraph DivideInitial["Dividing the Initial Sound"]
      D1 --> |Form|F[Initial Sound]
      E1 --> |Form|F
    end
    subgraph DivideSubsequent["Dividing the Subsequent Sound"]
      D2 --> |Form|G[Subsequent Sound]
      E2 --> |Form|G
    end
  end

  subgraph ClassifyInitialAndSubsequent["Classifying the Initial Sound and the Subsequent Sound"]
    F --> |Classify|F1{Does the Initial Sound have a distinctive function?}
    F1 --> |Yes|H1[Substantial Initial Sound]
    F1 --> |No|H2[Insubstantial Initial Sound]
    G --> |Classify|F2{What type of final structure does the Subsequent Sound have?}
    F2 --> |Tri-Quality Final|G1[Tri-Quality Subsequent Sound]
    F2 --> |Front-Long Final|G2[Front-Long Subsequent Sound]
    F2 --> |Back-Long Final|G3[Back-Long Subsequent Sound]
    F2 --> |Single-Quality Final|G4[Single-Quality Subsequent Sound]
  end

  subgraph AnalyzeInitialAndSubsequent["Analyzing the Initial Sound and the Subsequent Sound"]
    subgraph InitialAnalysis["Initial Sound Analysis"]
      H1 --> |Extract|H3[Pitch]
      H1 --> |Extract|H4[Quality]
      H2 --> |Extract|H5[Pitch]
      H2 --> |Extract|H6[Quality]
      H3 --> |Generalize as|H7[Pitch of the Initial Sound]
      H5 --> |Generalize as|H7
      H4 --> |Generalize as|H8[Quality of the Initial Sound]
      H6 --> |Generalize as|H8
      H7 --> |Analyze|H9[No stable pitch, non-distinctive feature]
      H8 --> |Analyze|H10[Stable quality, distinctive feature]
    end

    subgraph SubsequentAnalysis["Subsequent Sound Analysis"]
      subgraph SingleQualityAnalysis["Single-Quality Subsequent Sound Analysis"]
      G4 --> |Extract|J1[Subsequent Tonal Segment]
      G4 --> |Extract|J2[Single-Quality Final]
      J2 --> |Further analyze as|J3[Final Base<br>Final Body<br>Nucleus<br>Nuclear Core]
      J3 --> |Segment|J4[Anterior segment of the Final]
      J3 --> |Segment|J5[Middle segment of the Final]
      J3 --> |Segment|J6[Posterior segment of the Final]

      J1 --> |Transfer|J7[Subsequent Tonal Segment]
      J7 --> |Segment|J8[Second Pitch]
      J7 --> |Segment|J9[Main Pitch]
      J7 --> |Segment|J10[Last Pitch]
      end

      subgraph BackLongAnalysis["Back-Long Subsequent Sound Analysis"]
      G3 --> |Extract|K1[Subsequent Tonal Segment]
      G3 --> |Extract|K2[Back-Long Final]

      K2 --> |Segment|K3[Medial<br>Further analyzed as the head of the final]
      K2 --> |Segment|K4[Final Base<br>Final Body<br>Nucleus<br>Nuclear Core<br>Further analyzed as Rime Quality]
      K3 --> |Transfer|K5[Head of the Final]
      K4 --> |Segment|K6[Anterior segment of the Rime Quality<br>Anterior segment of the Nuclear Core]
      K4 --> |Segment|K7[Posterior segment of the Rime Quality<br>Posterior segment of the Nuclear Core]

      K1 --> |Segment|K8[Second Pitch]
      K1 --> |Segment|K9[Rime Pitch]
      K8 --> |Transfer|K10[Second Pitch]
      K9 --> |Segment|K11[Main Pitch]
      K9 --> |Segment|K12[Last Pitch]
      end

      subgraph FrontLongAnalysis["Front-Long Subsequent Sound Analysis"]
      G2 --> |Extract|L1[Subsequent Tonal Segment]
      G2 --> |Extract|L2[Front-Long Final<br>Further analyzed as Final Base or Final Body]

      L2 --> |Segment|L3[Nucleus<br>Nuclear Core<br>Further analyzed as Intermediate Quality]
      L2 --> |Segment|L4[Coda]
      L1 --> |Segment|L5[Intermediate Pitch]
      L1 --> |Segment|L6[Last Pitch]

      L3 --> |Segment|L7[Anterior segment of the Intermediate Quality<br>Anterior segment of the Nuclear Core]
      L3 --> |Segment|L8[Posterior segment of the Intermediate Quality<br>Posterior segment of the Nuclear Core]
      L4 --> |Transfer|L9[Coda]

      L5 --> |Segment|L10[Second Pitch]
      L5 --> |Segment|L11[Main Pitch]
      L6 --> |Transfer|L12[Last Pitch]
      end

      subgraph TriQualityAnalysis["Tri-Quality Subsequent Sound Analysis"]
      G1 --> |Extract|M1[Subsequent Tonal Segment]
      G1 --> |Extract|M2[Tri-Quality Final]

      M1 --> |Segment|M3[Second Pitch]
      M1 --> |Segment|M4[Rime Pitch]

      M3 --> |Transfer|M5[Second Pitch]
      M4 --> |Segment|M6[Main Pitch]
      M4 --> |Segment|M7[Last Pitch]

      M2 --> |Segment|M8[Medial]
      M2 --> |Segment|M9[Final Base<br>Final Body<br>Rime Quality]
      M8 --> |Further analyze as|M10[Head of the Final]
      M9 --> |Segment|M11[Nucleus<br>Nuclear Core]
      M9 --> |Segment|M12[Coda]
      end
    end
  end

  H9 --> |Abbreviate as|H11[Initial Tonal Segment]
  H10 --> |Abbreviate as|H12[Initial Quality]

  J8 --> |Transfer|J11[Second Pitch]
  J9 --> |Transfer|J12[Main Pitch]
  J10 --> |Transfer|J13[Last Pitch]
  J4 --> |Rename as|J14[Second Quality]
  J5 --> |Rename as|J15[Main Quality]
  J6 --> |Rename as|J16[Last Quality]

  K10 --> |Transfer|K13[Second Pitch]
  K11 --> |Transfer|K14[Main Pitch]
  K12 --> |Transfer|K15[Last Pitch]
  K5 --> |Rename as|K16[Second Quality]
  K6 --> |Rename as|K17[Main Quality]
  K7 --> |Rename as|K18[Last Quality]

  L10 --> |Transfer|L13[Second Pitch]
  L11 --> |Transfer|L14[Main Pitch]
  L12 --> |Transfer|L15[Last Pitch]
  L7 --> |Rename as|L16[Second Quality]
  L8 --> |Rename as|L17[Main Quality]
  L9 --> |Rename as|L18[Last Quality]

  M5 --> |Transfer|M13[Second Pitch]
  M6 --> |Transfer|M14[Main Pitch]
  M7 --> |Transfer|M15[Last Pitch]
  M10 --> |Rename as|M16[Second Quality]
  M11 --> |Rename as|M17[Main Quality]
  ...
  subgraph SyllableStructureModel["Syllable Structure Model"]
    N1 --> |Fill the Initial Sound|R1[Initial Sound]
    N2 --> |Fill the Second Sound|R2[Second Sound]
    N3 --> |Fill the Main Sound|R3[Main Sound]
    N4 --> |Fill the Last Sound|R4[Last Sound]

    O1 --> |Fill the Second Sound|R2
    O2 --> |Fill the Main Sound|R3
    O3 --> |Fill the Last Sound|R4

    P1 --> |Fill the Second Sound|R2
    P2 --> |Fill the Main Sound|R3
    P3 --> |Fill the Last Sound|R4

    Q1 --> |Fill the Second Sound|R2
    Q2 --> |Fill the Main Sound|R3
    Q3 --> |Fill the Last Sound|R4

    R1 --> |Promote|S1[Initial Sound]
    R2 --> |Promote|S2[Second Sound]
    R3 --> |Combine into|S3[Rime]
    R4 --> |Combine into|S3

    S1 --> |Promote|T1[Initial Sound]
    S2 --> |Combine into|T2[Subsequent Sound]
    S3 --> |Combine into|T2

    T1 --> |Combine into|U1[Syllable]
    T2 --> |Combine into|U1
  end
  U1 --> |Output|End((End))
```
