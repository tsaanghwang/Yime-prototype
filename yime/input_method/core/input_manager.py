...
class InputManager:
    ...
    def process_key(self, key_info: Dict[str, Any]) -> bool:
        ...
        if (current_time - self.state.last_key_time) > self.timeout:
            ...
            if self.state.buffer:
                print(f"输入超时，清空缓冲区: {self.state.buffer}")
            self.clear_buffer()
        ...
        # 调试输出
        # print(f"按键: {key}, ASCII: {ascii_char}")
    ...
    def add_char(self, char: str) -> None:
        ...
        if len(self.state.buffer) >= self.max_buffer_length:
            print(f"缓冲区已满，清空: {self.state.buffer}")
            self.clear_buffer()
        
        self.state.buffer += char
        self.state.is_composing = True
        
        print(f"添加字符: {char}, 缓冲区: {self.state.buffer}")
    ...
    def backspace(self) -> None:
        ...
        if self.state.buffer:
            ...
            print(f"Backspace: 缓冲区: {self.state.buffer}")
    ...
    def _update_candidates(self) -> None:
        ...
        try:
            ...
            # 保存候选词
            self.current_candidates = candidates[:9]  # 最多9个候选词
            ...
            print(f"候选词更新: {len(self.current_candidates)} 个")
