...
def test_repair_phrase_entries_normalizes_yime_code_and_drops_duplicates(tmp_path: Path) -> None:
...
def test_repair_candidate_frequency_entries_merges_trimmed_duplicates_and_drops_invalid(tmp_path: Path) -> None:
    store = UserLexiconStore(tmp_path / "user_lexicon.db")
...
def test_repair_meta_entries_clears_stale_seed_skip_without_user_data(tmp_path: Path) -> None:
    store = UserLexiconStore(tmp_path / "user_lexicon.db")
